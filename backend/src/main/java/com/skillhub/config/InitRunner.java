...
import com.skillhub.service.CommunitySeedService;
...
/**
 * 显式按顺序：(1) 建表 (2) 扫描 seniors 目录 (3) 社区示例数据首次导入 (4) 演示数据。
...
public class InitRunner {
    ...
    private final CommunitySeedService communitySeed;
    private final DemoDataSeeder demoData;

    public InitRunner(SqliteSchema schema, SeniorReader reader,
                      CommunitySeedService communitySeed, DemoDataSeeder demoData) {
        this.schema = schema;
        this.reader = reader;
        this.communitySeed = communitySeed;
